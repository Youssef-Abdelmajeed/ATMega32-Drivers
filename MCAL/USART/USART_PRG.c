...
uint8_t UCSRC_TEMP = 0b10000000 ; 

/* number of data to send/Receive */
uint8_t global_No_of_Bits = USART_8bit  ; 
...
static USART_SetStopBits(uint8_t StopBits)
{
    uint8_t errorState  ; 
    if(StopBits>1)
    {
...
{
    /* initialized UCSRC placeholder and select UC */
    UCSRC_TEMP = UCSRC_MASK ; 
...
{   uint8_t errorState  ; 
    uint16_t UBRR_Value = (FOSC /16* baudRate) -1 ; 
    if (UBRR_Value>4095)
    {
